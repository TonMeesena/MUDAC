...
train_summary_judgment %>%
  mutate(predictions = predict(tree_summary_judgment, newdata = train_summary_judgment, type = "class")) %>%
  summarize(error = mean(summary_judgment != predictions))
train_settled %>%
  mutate(predictions = predict(tree_settled, newdata = train_settled, type = "class")) %>%
  summarize(error = mean(settled != predictions))
train_dismissed %>%
  mutate(predictions = predict(tree_dismissed, newdata = train_dismissed, type = "class")) %>%
  summarize(error = mean(summary_judgment != predictions))
```
...
```{r}
demographics_lm <- glm(outcome ~ circut + district + office + )
```
