...
df.freq_full_FIPS_for_plotting <- df.freq_full_FIPS %>% dplyr::select(c(State, TotalPop, Hispanic, White, Native, Asian, Citizen, IncomePerCap, Poverty, Construction, Carpool,Employed,PrivateWork,SelfEmployed,Unemployment,highOrLow,Freq)) %>%
...
ggplot(df.freq_full_FIPS_for_plotting, aes(x=output, y=highOrLow, color  = Freq)) + geom_point() + 
  facet_wrap(~ variable) + 
  binomial_smooth()
```
